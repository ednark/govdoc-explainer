...
- `govdoc_explainer/cli.py` — entry point with `build` (default) and `profile` subcommands; `build --only "name,substr"` limits a run to matching sources
- `govdoc_explainer/config.py` — loads sources.csv, sources-*.csv, manual docs dir, perspectives(.csv + prompts/), llm.txt, company profile with _default fallback; exposes `profile_hash` and `perspectives_source`
...
- `govdoc_explainer/summarize.py` — LLM summary generation (overall/punchline/actions.<Role>/keywords + executive brief + oversized-document digest) and the artifact manifest
- `govdoc_explainer/render.py` — HTML page generation (per-document pages, search-first home, configs.html) + lunr index
...
- `config/prompts/*.txt` — prompt templates (system_context, overall, punchline, actions, keywords, exec_brief, digest_part, digest_reduce, company_profile, perspectives)
...
- `index.html` — search-first home page (search box + link to configs)
- `configs.html` — the company profile and LLM prompt settings behind the summaries (generated by `generate_configs_page`)
- `sources/<doc>/index.html` — one page per document (exec brief, Source Data with per-doc semantic search, role action summaries)
- The active company profile is shown in a "Company Profile" accordion on every page
